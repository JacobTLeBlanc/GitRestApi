...
resource "aws_lambda_permission" "apigw_get_repos" {
  ...
  source_arn = "arn:aws:execute-api:${var.region}:*:${aws_api_gateway_rest_api.git.id}/*/${aws_api_gateway_method.get_repos_method.http_method}${aws_api_gateway_resource.get_repos_resource.path}"
}